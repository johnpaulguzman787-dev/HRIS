...
// ══════════════════════════════════════════════════════════════════════
    ...
    public function pendingRequests(Request $request)
    {
        ...
        $adjustQuery = AttendanceAdjustmentRequest::with(['employee.department'])
            ->whereIn('status', ['pending', 'supervisor_approved']);
    ...
    private function applyAttendanceAdjustment(AttendanceAdjustmentRequest $aar): void
    {
        ...
        $shift = $log->shift ?? \App\Models\EmployeeShift::with('shift')
            ->where('employee_id', $aar->employee_id)
            ->where('is_active', true)
            ->latest('effective_date')
            ->first()?->shift;

        $isNightShift = $shift && $shift->end_time < $shift->start_time;
        if ($clockOut && $clockOut->lte($clockIn) && $isNightShift) {
            $clockOut->addDay();
        }
