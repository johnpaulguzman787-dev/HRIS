...
<div class="flex min-h-screen" x-data="approvedLogs()">
    ...
    <aside class="bg-white border-r border-gray-200 h-screen fixed left-0 top-0 overflow-y-auto z-50 flex flex-col"
        x-data="{
            sc: localStorage.getItem('sidebarCollapsed') === 'true',
            empOpen:  {{ $isEmployeesSection  ? 'true' : 'false' }},
            attOpen:  {{ $isAttendanceSection ? 'true' : 'false' }},
            payOpen:  {{ $isPayrollSection    ? 'true' : 'false' }},
            reqOpen:  {{ $isRequestsSection   ? 'true' : 'false' }}
        }"
        x-init="$watch('sc', v => localStorage.setItem('sidebarCollapsed', v))"
        :class="sc ? 'w-20' : 'w-64'"
        style="transition:width 0.35s cubic-bezier(0.4,0,0.2,1); box-shadow:2px 0 20px rgba(0,0,0,0.06);">

        {{-- Logo --}}
        <div class="px-6 py-5 border-b border-gray-100">
            <div class="flex items-center space-x-3" :class="sc ? 'justify-center' : ''">
                ...
                <h1 x-show="!sc" class="font-bold text-gray-900 text-lg tracking-widest whitespace-nowrap">MEDISOURCE</h1>
            </div>
        </div>

        {{-- User --}}
        <div class="px-4 py-4 border-b border-gray-100" :class="sc ? 'flex justify-center' : 'flex items-center space-x-3'">
            <div class="w-11 h-11 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 text-sm avatar-ring" style="background:linear-gradient(135deg,#3b82f6,#1d4ed8);">{{ $sidebarInitials }}</div>
            <div x-show="!sc" class="overflow-hidden">
        ...
        <nav class="p-3 space-y-0.5 flex-1 overflow-y-auto">
            <p x-show="!sc" class="text-xs text-gray-400 font-semibold px-3 py-2 uppercase tracking-widest">Main Menu</p>
            ...
            <a href="{{ route('admin.dashboard') }}" class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ $currentRoute==='admin.dashboard' ? 'text-white' : 'text-gray-600 hover:bg-gray-50' }}" style="{{ $currentRoute==='admin.dashboard' ? 'background:#3b82f6;' : '' }}">
                ...
                <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Dashboard</span>
            </a>

            {{-- Employees --}}
            <div>
                <button @click="empOpen=!empOpen" class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg {{ $isEmployeesSection ? 'text-white' : 'text-gray-600 hover:bg-gray-50' }}" style="{{ $isEmployeesSection ? 'background:#3b82f6;' : '' }}">
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                        <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Employees</span>
                    </div>
                    <svg x-show="!sc" class="w-4 h-4 chevron-icon" :class="{'rotate-180':empOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div x-show="empOpen&&!sc" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="ml-8 mt-1 space-y-0.5">
            ...
            {{-- Time & Attendance --}}
            <div>
                <button @click="attOpen=!attOpen" class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg {{ $isAttendanceSection ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Time & Attendance</span>
                    </div>
                    <svg x-show="!sc" class="w-4 h-4 chevron-icon" :class="{'rotate-180':attOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div x-show="attOpen&&!sc" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="ml-8 mt-1 space-y-0.5">
            ...
            {{-- Payroll --}}
            <div>
                <button @click="payOpen=!payOpen" class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg {{ $isPayrollSection ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Payroll</span>
                    </div>
                    <svg x-show="!sc" class="w-4 h-4 chevron-icon" :class="{'rotate-180':payOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div x-show="payOpen&&!sc" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="ml-8 mt-1 space-y-0.5">
            ...
            {{-- Requests & Approval --}}
            <div>
                <button @click="reqOpen=!reqOpen" class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg {{ $isRequestsSection ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:bg-gray-50' }}">
                    <div class="flex items-center space-x-3">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                        <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Requests &amp; Approval</span>
                    </div>
                    <svg x-show="!sc" class="w-4 h-4 chevron-icon" :class="{'rotate-180':reqOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div x-show="reqOpen&&!sc" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="ml-8 mt-1 space-y-0.5">
            ...
            <div class="pt-3 mt-2 border-t border-gray-100">
                <p x-show="!sc" class="text-xs text-gray-400 font-semibold px-3 py-2 uppercase tracking-widest">Others</p>
                <a href="{{ route('settings.index') }}" class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ $currentRoute==='settings.index' ? 'text-white' : 'text-gray-600 hover:bg-gray-50' }}" style="{{ $currentRoute==='settings.index' ? 'background:#3b82f6;' : '' }}">
                    ...
                    <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Settings</span>
                </a>
                <a href="{{ route('logout') }}" onclick="event.preventDefault();document.getElementById('admin-logout-form').submit();" class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-600 hover:bg-red-50 hover:text-red-500">
                    ...
                    <span x-show="!sc" class="text-sm font-medium whitespace-nowrap">Logout</span>
                </a>
                <form id="admin-logout-form" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
            </div>
        </nav>

        <button @click="sc=!sc" class="m-3 w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-200 self-end">
            <svg class="w-4 h-4 chevron-icon" :class="{'rotate-180':sc}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/></svg>
        </button>
    </aside>
    ...
    <div class="flex-1 flex flex-col" style="margin-left:256px;">
        ...
        {{-- Top bar --}}
        <div style="background:linear-gradient(135deg,#3b82f6,#1d4ed8);padding:22px 32px;" class="flex items-center justify-between">
            <h1 class="text-white font-bold text-xl">Approved Logs</h1>
            <button class="relative w-9 h-9 flex items-center justify-center rounded-full" style="background:rgba(255,255,255,0.15);">
