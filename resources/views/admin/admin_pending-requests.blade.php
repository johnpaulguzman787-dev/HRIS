...
<head>
    ...
    <style>
        *{font-family:'DM Sans',sans-serif;box-sizing:border-box;}
        ...
        @media (min-width: 640px) {
            .btn-cancel{padding:8px 18px;width:auto;}
            .btn-save{padding:8px 20px;width:auto;}
        }
    ...
    <header class="anim-fade bg-gradient-to-br from-blue-500 to-blue-700 sticky top-0 z-10 shadow-lg mt-3 mx-3 lg:mt-4 lg:mx-4 rounded-2xl">
        <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <button @click="mobileMenuOpen = true" class="lg:hidden p-1.5 rounded-lg hover:bg-white/20 transition-colors">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-blue-100 mt-1">Review and manage employee requests</p>
                </div>
            </div>
            ...
            <div x-show="reqType==='leave'" x-transition
                 x-data="{ leaveTypeId:'', startDate:'', endDate:'', reason:'', fileName:'', saving:false, errorMsg:'',
                     ...
                     async submit(){
                         ...
                         const fi=document.getElementById('adminReqLeaveDoc');
                         ...
                         if(res.ok){ $root.showFileReq=false; $root.reqType=''; $root.resultType='success'; $root.resultTitle='Leave Request Filed'; $root.resultMessage='Ref: '+(data.ref_no??''); $root.showResult=true; setTimeout(()=>window.location.reload(),2500); }
            ...
            <div x-show="reqType==='overtime'" x-transition
                 x-data="{ otDate:'', otStart:'', otEnd:'', reason:'', fileName:'', saving:false, errorMsg:'',
                     ...
                     async submit(){
                         ...
                         form.append('reason',this.reason);
                         const fi=document.getElementById('adminReqOtDoc');
                         if(fi.files[0]) form.append('document',fi.files[0]);
                         form.append('_token',document.querySelector('meta[name=csrf-token]').content);
                         const res=await fetch('{{ route('admin.requests.overtime.file') }}',{method:'POST',body:form});
                         this.saving=false;
                         const data=await res.json();
                         if(res.ok){ $root.showFileReq=false; $root.reqType=''; $root.resultType='success'; $root.resultTitle='Overtime Request Filed'; $root.resultMessage='Ref: '+(data.ref_no??''); $root.showResult=true; setTimeout(()=>window.location.reload(),2500); }
                         else{ this.errorMsg=data.message??'Something went wrong.'; }
                     }
                 }">
                <template x-if="errorMsg"><div style="background:#fee2e2;color:#b91c1c;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:14px;" x-text="errorMsg"></div></template>
            ...
            <div x-show="reqType==='shift'" x-transition
                 x-data="{ requestedShiftId:'', effectiveFrom:'', effectiveUntil:'', reason:'', fileName:'', saving:false, errorMsg:'',
                     ...
                     async submit(){
                         ...
                         form.append('reason',this.reason);
                         const fi=document.getElementById('adminReqShiftDoc');
                         if(fi.files[0]) form.append('document',fi.files[0]);
                         form.append('_token',document.querySelector('meta[name=csrf-token]').content);
                         const res=await fetch('{{ route('admin.requests.shift.file') }}',{method:'POST',body:form});
                         this.saving=false;
                         const data=await res.json();
                         if(res.ok){ $root.showFileReq=false; $root.reqType=''; $root.resultType='success'; $root.resultTitle='Shift Request Filed'; $root.resultMessage='Ref: '+(data.ref_no??''); $root.showResult=true; setTimeout(()=>window.location.reload(),2500); }
                         else{ this.errorMsg=data.message??'Something went wrong.'; }
                     }
                 }">
                <template x-if="errorMsg"><div style="background:#fee2e2;color:#b91c1c;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:14px;" x-text="errorMsg"></div></template>
