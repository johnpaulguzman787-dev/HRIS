...
<div x-data="{ collapsed: localStorage.getItem('sidebarCollapsed') === 'true' }"
    ...
    <header class="anim-fade bg-gradient-to-br from-blue-500 to-blue-700 sticky top-0 z-10 shadow-lg mt-3 mx-3 lg:mt-4 lg:mx-4 rounded-2xl">
        <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-white">Shift Scheduling</h1>
                    <p class="text-xs sm:text-sm text-blue-100 mt-1">Manage shifts, types and holidays</p>
                </div>
            </div>
    ...
    <div class="main-content-padding" style="padding:20px 16px;" x-data="{ showEditShiftModal: false, showAssignShiftModal: false, showAddShiftTypeModal: false, showAddHolidayModal: false }">
        ...
        <div class="table-card" x-data="{
            ...
            async openEdit(id) {
                this.errorMsg = '';
                const res = await fetch(`/admin/shift/type/${id}`, {
        ...
        <div class="table-card" x-data="{
            ...
            async openEdit(id) {
                this.errorMsg = '';
                const res = await fetch(`/admin/holidays/${id}`, {
                    headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content }
                });
                const data = await res.json();
                this.editHoliday = {
                    id: data.id, name: data.name, date: data.date,
                    type: data.type, pay_rate: data.pay_rate,
                    region: data.region ?? '', yearly: data.yearly ?? false,
                };
            ...
            async submitEdit() {
                this.errorMsg = '';
                this.saving = true;
                const res = await fetch(`/admin/holidays/${this.editHoliday.id}/update`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content },
        ...
        <div x-show="showEditShiftModal" class="modal-overlay" x-cloak @click.self="showEditShiftModal = false"
             x-data="{
                 empId: '', empShiftId: '', shiftId: '', workSetup: '',
                 effectiveDate: '', endDate: '', daysOff: ['Sat','Sun'],
                 currentInfo: null, saving: false, errorMsg: '',
                 async onEmpChange() {
                     this.currentInfo = null; this.empShiftId = '';
                     if (!this.empId) return;
                     const res = await fetch('{{ route('admin.shift.employees-by-dept') }}?employee_id=' + this.empId, {
                         headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content }
                     });
                     const data = await res.json();
                     if (data.shift) {
                 ...
                 async submit() {
                     ...
                     const res = await fetch('{{ route('admin.shift.update') }}', {
                         ...
                         headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content },
                         body: JSON.stringify({
                             employee_shift_id: this.empShiftId, shift_id: this.shiftId,
                             work_setup: this.workSetup, effective_date: this.effectiveDate,
                             end_date: this.endDate || null, days_off: this.daysOff,
                         })
                     });
                     this.saving = false;
                     const data = await res.json();
        ...
        <div x-show="showAssignShiftModal" class="modal-overlay" x-cloak @click.self="showAssignShiftModal = false"
             x-data="{
                 ...
                 async onDeptChange() {
                     ...
                     const res = await fetch('{{ route('admin.shift.employees-by-dept') }}?department_id=' + this.deptId, {
                         headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content }
                     });
                 ...
                 async submit() {
                     ...
                     this.saving = true;
                     const res = await fetch('{{ route('admin.shift.assign') }}', {
                         method: 'POST',
                         headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content },
                         body: JSON.stringify({
                             employee_id: this.empId, shift_id: this.shiftId,
                             work_setup: this.workSetup, effective_date: this.effectiveDate,
                             end_date: this.endDate || null, days_off: this.daysOff,
                         })
                     });
        ...
        <div x-show="showAddHolidayModal" class="modal-overlay" x-cloak @click.self="showAddHolidayModal = false"
             x-data="{
                 ...
                 async submit() {
                     ...
                     this.saving = true;
                     const res = await fetch('{{ route('admin.holidays.store') }}', {
                         method: 'POST',
                         headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content },
                         body: JSON.stringify({
