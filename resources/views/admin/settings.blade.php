...
<div x-data="{
    ...
    activeTab: '{{ $errors->any() ? "account" : "permissions" }}',
    saving: null,
...
class="flex h-screen overflow-hidden bg-gray-50">
    ...
    <main class="flex-1 overflow-y-auto min-h-screen transition-all duration-300"
        ...
        <div class="p-3 lg:p-6">
            ...
            <!-- Tab Navigation -->
            <div class="flex gap-1 mb-6 bg-white rounded-xl border border-gray-200 shadow-sm p-1 w-fit">
                <button @click="activeTab='permissions'"
                        :class="activeTab==='permissions' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                        class="px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-150">
                    Roles &amp; Permissions
                </button>
                <button @click="activeTab='account'"
                        :class="activeTab==='account' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                        class="px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-150">
                    Account
                </button>
            </div>
            ...
            <div x-show="activeTab==='account'" x-cloak>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden max-w-xl">
...
<div x-show="showModal" x-cloak
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden"
        ...
        <form method="POST" action="{{ route('admin.settings.password') }}" class="px-7 py-6 space-y-5">
            @csrf
