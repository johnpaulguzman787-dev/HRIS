@php
    ...
    $payrollRoutes    = ['employee.payslips', 'employee.govpay'];
...
<aside
    ...
    <nav class="p-3 space-y-0.5 flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <!-- Time & Attendance -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? attendanceOpen = !attendanceOpen : window.location='{{ route('employee.attendance.reports') }}'"
        ...
        <!-- Payroll -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? payrollOpen = !payrollOpen : window.location='{{ route('employee.payslips') }}'"
            ...
            <div x-show="payrollOpen && (isMobile || !sidebarCollapsed)"
                 ...
                 x-transition:leave-end="opacity-0 -translate-y-3 scale-y-95"
                 class="ml-8 mt-1 space-y-0.5 origin-top">
                <a href="{{ route('employee.payslips') }}" class="submenu-item block px-3 py-2 text-sm rounded-lg {{ $currentRoute === 'employee.payslips' ? 'text-blue-600 font-semibold bg-blue-50' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50' }}">Payslips</a>
                <a href="{{ route('employee.govpay') }}" class="submenu-item block px-3 py-2 text-sm rounded-lg {{ $currentRoute === 'employee.govpay' ? 'text-blue-600 font-semibold bg-blue-50' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50' }}">Govt. Contributions</a>
            </div>
        </div>

        <!-- Requests & Approval -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? requestsOpen = !requestsOpen : window.location='{{ route('employee.requests.pending') }}'"
                class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-gray-600 hover:bg-gray-50">
                <div class="flex items-center space-x-3">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="pt-3 mt-2 border-t border-gray-100">
            ...
            <a href="{{ route('employee.settings') }}" class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ $currentRoute === 'employee.settings' ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50' }} group">
