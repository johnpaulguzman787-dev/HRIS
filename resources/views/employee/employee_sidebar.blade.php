...
<aside
    ...
    <nav class="p-3 space-y-0.5 flex-1 overflow-hidden">
        ...
        <!-- Time & Attendance -->
        <div>
            <button @click="sidebarCollapsed ? window.location='{{ route('employee.attendance.reports') }}' : attendanceOpen = !attendanceOpen"
        ...
        <!-- Payroll -->
        <div>
            <button @click="sidebarCollapsed ? window.location='{{ route('employee.payslips') }}' : payrollOpen = !payrollOpen"
        ...
        <!-- Requests & Approval -->
        <div>
            <button @click="sidebarCollapsed ? window.location='{{ route('employee.requests.pending') }}' : requestsOpen = !requestsOpen"
                class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-gray-600 hover:bg-gray-50">
                <div class="flex items-center space-x-3">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
