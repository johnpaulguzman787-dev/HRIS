...
<body x-data="payrollApp()" x-init="init()" class="flex h-screen overflow-hidden bg-gray-50">
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs sm:text-sm text-blue-100 mt-1">Payroll Management</p>
                    </div>
                </div>
        ...
        <div x-show="page === 'list'" x-cloak>
                    ...
                    <div class="pb-4">
                        ...
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                            <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
                                <table class="data-table">
                                    <thead>
                                        <tr class="border-t border-b border-gray-100 bg-gray-50/60"><th>Grade Code</th><th>Level Name</th><th>Monthly Basic Salary</th><th>Semi-Monthly Pay</th><th>Assigned Employees</th><th></th></tr>
                                    </thead>
                                    <tbody>
                ...
                <div>
                    ...
                    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
                            <table class="data-table">
                                <thead>
                                    <tr class="border-t border-b border-gray-100 bg-gray-50/60"><th>Payroll Item</th><th>Multiplier</th><th>Type</th><th>Basis</th><th>Status</th><th></th></tr>
                                </thead>
                                <tbody>
            ...
            <div x-show="activeTab==='benefits'" x-cloak class="tab-content">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <table class="data-table">
                        <thead>
                            <tr class="border-t border-b border-gray-100 bg-gray-50/60"><th>Benefit Item</th><th>Type</th><th>Amount</th><th>Taxable</th><th>Frequency</th><th>Eligibility</th><th>Status</th><th></th></tr>
                        </thead>
                        <tbody>
            ...
            <div x-show="activeTab==='contributions'" x-cloak class="tab-content space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                        <div class="flex items-center justify-between mb-4">
                            <h4 class="text-sm font-bold text-gray-700">PhilHealth</h4>
                        ...
                        <div class="space-y-2.5 text-sm">
                            ...
                            <div class="flex justify-between pt-2.5 border-t border-gray-100"><span class="text-gray-500">Salary Floor</span><span class="font-medium text-gray-700">₱{{ number_format($contrib['philhealth_floor']) }}.00</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">Salary Ceiling</span><span class="font-medium text-gray-700">₱{{ number_format($contrib['philhealth_ceiling']) }}.00</span></div>
                        </div>
                    </div>
    ...
    <div x-show="showAddItemModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center modal-overlay" @click.self="showAddItemModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-7"
            ...
            <form action="{{ route('finance_officer.payroll.item.store') }}" method="POST" class="space-y-4">
                @csrf
    ...
    <div x-show="showEditItemModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center modal-overlay" @click.self="showEditItemModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-7"
            ...
            <form :action="`/finance_officer/payroll/item/${editItem.id}/update`" method="POST" class="space-y-4">
                @csrf @method('PUT')
    ...
    <div x-show="showAddBenefitModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center modal-overlay" @click.self="showAddBenefitModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-7"
            ...
            <form action="{{ route('finance_officer.payroll.benefit.store') }}" method="POST" class="space-y-4">
                @csrf
    ...
    <div x-show="showEditBenefitModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center modal-overlay" @click.self="showEditBenefitModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-7"
            ...
            <form :action="`/finance_officer/payroll/benefit/${editBenefit.id}/update`" method="POST" class="space-y-4">
                @csrf @method('PUT')
    ...
    <div x-show="showAssignBenefitModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center modal-overlay" @click.self="showAssignBenefitModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-7"
            ...
            <form :action="`/finance_officer/payroll/benefit/${assignBenefit.id}/assign`" method="POST" class="space-y-4">
                @csrf
                <div>
    ...
    function payrollApp() {
        return {
            ...
            async savePeriodEdit() {
                ...
                try {
                    const res = await fetch(`/finance_officer/payroll/period/${ep.id}/update`, {
                        method: 'POST',
                        headers: {
            ...
            deleteGrade(id) {
                this.askConfirm('Delete Salary Grade', 'Assigned employees will be unassigned. This cannot be undone.', () => {
                    fetch(`/finance_officer/payroll/grade/${id}/delete`, {
            ...
            deleteItem(id) {
                this.askConfirm('Delete Payroll Item', 'This will permanently remove this payroll item.', () => {
                    fetch(`/finance_officer/payroll/item/${id}/delete`, {
                        method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
                    }).then(() => this.reloadTab());
                });
            },
            deactivateItem() {
                ...
                this.askConfirm(`${label} Payroll Item`, `This will ${label.toLowerCase()} "${this.editItem.name}".`, () => {
                    fetch(`/finance_officer/payroll/item/${this.editItem.id}/deactivate`, {
            ...
            deleteBenefit(id) {
                this.askConfirm('Delete Benefit', 'This will permanently remove this benefit.', () => {
                    fetch(`/finance_officer/payroll/benefit/${id}/delete`, {
                        method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
                    }).then(() => this.reloadTab());
                });
            },
            deactivateBenefit() {
                ...
                this.askConfirm(`${label} Benefit`, `This will ${label.toLowerCase()} "${this.editBenefit.name}".`, () => {
                    fetch(`/finance_officer/payroll/benefit/${this.editBenefit.id}/deactivate`, {
                        method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
                    }).then(() => this.reloadTab());
            ...
            async saveContrib() {
                ...
                if (this.editContribType === 'sss') {
                    await fetch('/finance_officer/payroll/sss/save', {
                        method: 'POST',
                        headers,
                    ...
                    await fetch('/finance_officer/payroll/contrib/update', {
                        method: 'POST',
                        headers,
