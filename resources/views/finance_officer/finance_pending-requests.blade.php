@php
    ...
    $employeeRoutes   = ['finance_officer.dashboard','finance_officer.profile'];
        ...
        <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-blue-100 mt-1">My Request</p>
                </div>
            </div>
    ...
    <div class="page-content">
        ...
        <div class="rcard">
            ...
            <div class="action-row">
                <button class="btn-cancel-req" @click="selId={{ $req->id }};selName='{{ addslashes(trim((auth()->user()->employee->fname ?? '').' '.(auth()->user()->employee->lname ?? ''))) }}';selCancelUrl='{{ route('finance_officer.requests.cancel', $req->id) }}';selType='{{ $req->type }}';showCancel=true">
            ...
            <div x-show="reqType==='overtime'" x-transition
                 x-data="{ otDate:'', otStart:'', otEnd:'', reason:'', fileName:'', saving:false, errorMsg:'',
                     ...
                     async submit(){
                         ...
                         if(fi.files[0]) form.append('document',fi.files[0]);
                         form.append('_token',document.querySelector('meta[name=csrf-token]').content);
                         const res=await fetch('{{ route('finance_officer.requests.overtime.file') }}',{method:'POST',body:form});
                         this.saving=false;
                         const data=await res.json();
            ...
            <div x-show="reqType==='shift'" x-transition
                 x-data="{ requestedShiftId:'', effectiveFrom:'', effectiveUntil:'', reason:'', fileName:'', saving:false, errorMsg:'',
                     ...
                     async submit(){
                         ...
                         if(fi.files[0]) form.append('document',fi.files[0]);
                         form.append('_token',document.querySelector('meta[name=csrf-token]').content);
                         const res=await fetch('{{ route('finance_officer.requests.shift.file') }}',{method:'POST',body:form});
                         this.saving=false;
                         const data=await res.json();
