@php
    ...
    $attendanceRoutes = ['finance_officer.attendance.reports', 'finance_officer.attendance.employee', 'finance_officer.attendance.shift', 'finance_officer.attendance.leave', 'finance_officer.leave.management'];
...
<aside
    ...
    <!-- Logo -->
    <div class="py-5 border-b border-gray-100" :class="(!isMobile && sidebarCollapsed) ? 'px-2' : 'px-6'">
    ...
    <!-- User Profile -->
    <div class="px-4 py-4 border-b border-gray-100 profile-card" :class="(!isMobile && sidebarCollapsed) ? 'flex justify-center' : 'flex items-center space-x-3'">
    ...
    <nav class="p-3 space-y-0.5 flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <a href="{{ route('finance_officer.profile') }}"
            ...
            <span x-show="isMobile || !sidebarCollapsed" class="text-sm font-medium whitespace-nowrap">Employee Profile</span>
        </a>

        <!-- Employee Directory -->
        <a href="{{ route('finance_officer.employees.directory') }}"
            class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg
                {{ $currentRoute === 'finance_officer.employees.directory' ? 'text-white' : 'text-gray-600 hover:bg-gray-50' }}"
            style="{{ $currentRoute === 'finance_officer.employees.directory' ? 'background:#3b82f6;' : '' }}">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
            <span x-show="isMobile || !sidebarCollapsed" class="text-sm font-medium whitespace-nowrap">Employee Directory</span>
        </a>
        ...
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? attendanceOpen = !attendanceOpen : (window.location.href='{{ route('finance_officer.attendance.reports') }}')"
                ...
                <svg x-show="isMobile || !sidebarCollapsed" class="w-4 h-4 chevron-icon" :class="{'rotate-180': attendanceOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-show="attendanceOpen && (isMobile || !sidebarCollapsed)"
                ...
                <a href="{{ route('finance_officer.attendance.reports') }}"
                   class="submenu-item flex items-center px-3 py-2 text-sm rounded-lg
                       {{ $currentRoute === 'finance_officer.attendance.reports'
        ...
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? payrollOpen = !payrollOpen : (window.location.href='{{ route('finance_officer.payroll') }}')"
                ...
                <svg x-show="isMobile || !sidebarCollapsed" class="w-4 h-4 chevron-icon" :class="{'rotate-180': payrollOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
            </button>
        ...
        <!-- Requests & Approval -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? requestsOpen = !requestsOpen : window.location='{{ route('finance_officer.requests.pending') }}'"
                class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-gray-600 hover:bg-gray-50">
                <div class="flex items-center space-x-3">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg x-show="isMobile || !sidebarCollapsed" class="w-4 h-4 chevron-icon" :class="{'rotate-180': requestsOpen}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
            </button>
        ...
        <div class="pt-3 mt-2 border-t border-gray-100">
            ...
            <a href="{{ route('finance_officer.settings') }}" class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg {{ $currentRoute === 'finance_officer.settings' ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50' }} group">
            ...
            <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('finance-officer-logout-form').submit();"
            ...
            <form id="finance-officer-logout-form" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
        </div>
    </nav>
