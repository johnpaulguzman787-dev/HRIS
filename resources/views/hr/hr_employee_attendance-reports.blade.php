    ...
    <nav class="p-3 space-y-0.5 flex-1 overflow-y-auto">
        ...
        <div>
            ...
            <div x-show="employeesOpen && !sidebarCollapsed"
                ...
                <a href="#" class="block px-3 py-2 text-sm text-gray-500 hover:text-gray-800 hover:bg-gray-50 rounded-lg">Employee Directory</a>
                <a href="#" class="block px-3 py-2 text-sm text-gray-500 hover:text-gray-800 hover:bg-gray-50 rounded-lg">Employee Profile</a>
            </div>
        </div>
            ...
            <div x-show="attendanceOpen && !sidebarCollapsed"
                ...
                <a href="{{ route('hr.attendance.reports') }}"
                   class="flex items-center px-3 py-2 text-sm rounded-lg
                       {{ $currentRoute === 'hr.attendance.reports' ? 'text-blue-600 font-semibold' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50' }}">
                    @if($currentRoute === 'hr.attendance.reports')
                        <span class="w-2 h-2 rounded-full mr-2.5 flex-shrink-0 bg-blue-500" style="animation:pulseDot 2s ease-in-out infinite;"></span>
                    @endif
                    My Attendance
                </a>
...
<div x-data="{ collapsed: localStorage.getItem('sidebarCollapsed') === 'true' }"
    ...
    <div style="padding:24px 32px;">
        ...
        <div class="breadcrumb">
            <a href="{{ route('hr.attendance.employee', ['view' => 'monthly', 'month' => $selectedMonth]) }}">
        ...
        <div class="table-card">
            <div class="table-toolbar">
                ...
                <div class="toolbar-right">
                    <form method="GET" action="{{ route('hr.attendance.employee') }}" id="detailForm" style="display:contents;">
                    ...
                    {{-- Export --}}
                    <a href="{{ route('hr.attendance.employee', ['view' => 'monthly', 'employee_id' => request('employee_id'), 'month' => $selectedMonth, 'period' => $selectedPeriod, 'export' => 1]) }}"
                       class="export-btn">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="table-card">
            <div class="table-toolbar">
                <h2>Attendance Records</h2>
                <div class="toolbar-right">
                    <form method="GET" action="{{ route('hr.attendance.employee') }}" id="filterForm" style="display:contents">
                        ...
                        <tr>
                            <td>
                                <div style="display:flex; align-items:center; gap:10px;">
                                    ...
                                    <div>
                                        <div class="emp-name">{{ $rec->employee->name ?? 'Juan Dela Cruz' }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <span class="hours-pill ot">{{ $rec->overtime_formatted ?? '00h 00m' }}</span>
                            </td>
                            <td>
                                <span class="hours-pill ut">{{ $rec->undertime_formatted ?? '00h 00m' }}</span>
                            </td>
                            <td>
                ...
                <table class="monthly-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="display:flex; align-items:center; gap:10px;">
                                    ...
                                    <div><div class="emp-name">{{ $rec->employee->name ?? 'Juan Dela Cruz' }}</div></div>
                                </div>
                            </td>
                            <td class="emp-dept">{{ $rec->employee->department->name ?? '—' }}</td>
                            ...
                            <td>
                                <a href="{{ route('hr.attendance.employee', ['employee_id' => $rec->employee_id ?? '', 'view' => 'monthly', 'month' => request('month', now()->format('Y-m'))]) }}"
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('hr.attendance.employee', ['employee_id' => $i + 1, 'view' => 'monthly', 'month' => request('month', now()->format('Y-m'))]) }}"
                                   class="view-monthly-btn">
                                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
