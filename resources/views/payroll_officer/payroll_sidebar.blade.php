...
<aside
    ...
    <nav class="p-3 space-y-0.5 flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <!-- Employee Profile -->
        <a href="{{ route('payroll_officer.profile') }}"
            class="nav-item flex items-center space-x-3 px-3 py-2.5 rounded-lg
                {{ $currentRoute === 'payroll_officer.profile' ? 'text-white' : 'text-gray-600 hover:bg-gray-50' }}"
            style="{{ $currentRoute === 'payroll_officer.profile' ? 'background:#3b82f6;' : '' }}">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Time & Attendance -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? attendanceOpen = !attendanceOpen : window.location='{{ route('payroll_officer.attendance.reports') }}'"
        ...
        <!-- Payroll -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? payrollOpen = !payrollOpen : window.location='{{ route('payroll_officer.payroll') }}'"
                class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg
        ...
        <!-- Requests & Approval -->
        <div>
            <button @click="(isMobile || !sidebarCollapsed) ? requestsOpen = !requestsOpen : window.location='{{ route('payroll_officer.requests.pending') }}'"
                class="nav-item w-full flex items-center justify-between px-3 py-2.5 rounded-lg
        ...
        <div class="pt-3 mt-2 border-t border-gray-100">
            ...
            <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('payroll-officer-logout-form').submit();"
            ...
            <form id="payroll-officer-logout-form" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
        </div>
    </nav>
