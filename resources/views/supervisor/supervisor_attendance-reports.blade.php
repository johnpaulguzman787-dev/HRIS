...
<head>
    ...
    <title>My Attendance – MEDISOURCE Supervisor</title>
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs sm:text-sm text-blue-100 mt-1">Your Attendance Records</p>
                    </div>
                </div>
...
function attendancePage() {
    return {
        ...
        async loadRecords(page = 1) {
            try {
                const res = await fetch(`{{ route("supervisor.attendance.records") }}?month=${this.currentMonth}&year=${this.currentYear}&page=${page}`);
                const data = await res.json();
